...
const external = "I'm not outside the function";

function myFunction() {
  console.log(external);
  const internal = "Hello! I'm not inside myFunction!";
...
// Nested function can the access the internal variable because it is also on outside the curly brackets
...
function summation(bananas) {
  let sum = 0;
  for(let i = 1; i <= bananas; i++){
    sum += i;
  }
  return sum;
  }
...
const zooAnimals = [
 ...
 function animalNames(array){
  const displayNames = [];
  array.forEach(giraffe => displayNames.push(`name: ${giraffe.animal_name}, scientific: ${giraffe.scientific_name}`));
  return displayNames;
}
 ...
 function lowerCaseNames(arr){
  return arr.map(giraffe => giraffe.animal_name.toLowerCase());
}
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    return arr.filter(giraffe => giraffe.population < 5);
  }
  ...
  */
  ...
  function USApop(arr){
    return arr.reduce((total, giraffe) => total += giraffe.population, 0);
  }
